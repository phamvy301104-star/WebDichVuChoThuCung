...
export const ServiceListPage: React.FC = () => (
  <>
    ...
    <main className="page-container">
      <div style={{ textAlign: 'center', padding: '48px 20px 32px', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', borderRadius: 16, color: '#fff', marginBottom: 32 }}>
        ...
        {services.map(s => (
          <div key={s.id} style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', border: '1px solid #f0f0f0' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
              <span style={{ background: '#ede9fe', color: '#6d28d9', padding: '2px 8px', borderRadius: 6, fontSize: '0.78rem', fontWeight: 600 }}>{s.petType}</span>
              <span style={{ background: '#f0fdf4', color: '#166534', padding: '2px 8px', borderRadius: 6, fontSize: '0.78rem', fontWeight: 600 }}>⏱ {s.duration}</span>
            </div>
            <span style={{ fontWeight: 800, color: '#6366f1', fontSize: '1.05rem' }}>{s.price}</span>
          </div>
        ))}
      </div>
    </main>
